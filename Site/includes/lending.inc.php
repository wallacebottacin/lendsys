...
if (isset($_POST["makelending"])) {
    ...
    $quantityborrowed = mysqli_real_escape_string($conn, securityDataValidation($_POST['quantityborrowed']));
    ...
    if (empty($id) || empty($name) ||  empty($description) ||  empty($category) ||  empty($quantity) ||  empty($quantityborrowed)  || empty($emailuser)) {
    ...
    makeLending($conn, $id, $name, $description, $category, $quantity, $totalborrowed, $quantityborrowed, $emailuser);
